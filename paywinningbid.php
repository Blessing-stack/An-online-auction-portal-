...
$qsqlproduct = mysqli_query($con,$sqlproduct);
		...
		{
			if((is_countable($rsproduct['product_image'])?$rsproduct['product_image']:[]))
			{
				$product_image = unserialize($rsproduct['product_image']);
				if (file_exists("imgproduct/".$product_image[0])) 
				{
					 $imgname = "imgproduct/".$product_image[0];
				} 
				else 
				{
			...
			else
			{
				...
				if (file_exists("imgproduct/".$product_image)) 
				{
					 $imgname = "imgproduct/".$product_image;
				} 
				else 
				{
					$imgname = "images/noimage.gif";
...
?>
            ...
            <div class="product-grid8 border">
                <div class="product-image8">
                    <a href="single.php?productid=<?php echo $rsproduct['proid']; ?>">
                        <img class="pic-1" src="<?php echo $imgname; ?>" style="height: 400px;">
                        <img class="pic-2" src="<?php echo $imgname; ?>" style="height: 400px;">
                    </a>
        ...
        <div class="col-md-6 col-sm-6 ">
            <div class="product-grid8 border">
                <div class="product-image8">
                    <a href="single.php?productid=<?php echo $rsproduct['proid']; ?>">
                        <img class="pic-1" src="<?php echo $imgwinner; ?>" style="height: 400px;">
                        <img class="pic-2" src="<?php echo $imgwinner; ?>" style="height: 400px;">
                    </a>
                    <span class="product-discount-label"><b><?php echo $rsproduct['product_name']; ?></b></span>
                </div>
                <div class="product-content">
                    ...
                    <span class="product-shipping" style="color: green;"><b>Amount payable: : <?php echo $currencysymbol; ?> <?php echo $rsproduct['winning_bid']; ?></b></span>
                </div>
            </div>
        </div>
	</div>
	
<form action="" method="post" id="frmdata" name="frmdata" class="creditly-card-form agileinfo_form" enctype="multipart/form-data">
<input type='hidden' name='product_id' id="product_id" value='<?php echo $rsproduct['product_id']; ?>'>
<input type='hidden' name='customer_id' id="customer_id" value='<?php echo $rsproduct['customer_id']; ?>'>
<input type='hidden' name='winner_id' id="winner_id" value='<?php echo $_REQUEST['winner_id']; ?>'>
	<div class="row">
	<div class="col-md-12 col-sm-12 "><hr><center><h3>Payment Panel</h3></center><hr></div>
        <div class="col-md-12 col-sm-12 row">
            <div class="col-md-6">
				<label class="control-label">Customer name </label>
				<input name="custname" id="custname" class="form-control" type="text" readonly value='<?php echo $rscustomer['customer_fname']; ?> <?php echo $rscustomer['customer_lname']; ?>'>
				<br>
            </div>
            <div class="col-md-6">
				<label class="control-label">Upload Profile image <span class="errormsg"  id="errfile"></span></label>
				<input name="file" id="file" class="form-control" type="file" >
				<br>
            </div>
			
            <div class="col-md-12">
				<label class="control-label">Address <span class="errormsg"  id="erraddress"></span></label>
				<textarea name="address" id="address" class="form-control" placeholder="Enter Address"><?php echo $rscustomer['address']; ?></textarea>
				<br>
            </div>
            <div class="col-md-6">
				<label class="control-label">State <span class="errormsg"  id="errstate"></span></label>
				<input type="text"  name="state" id="state" class="form-control" placeholder="State" value="<?php echo $rscustomer['state']; ?>">
				<br>
            </div>
            <div class="col-md-6">
				<label class="control-label">City <span class="errormsg"  id="errcity"></span></label>
				<input type="text"  name="city" id="city" class="form-control" placeholder="City" value="<?php echo $rscustomer['city']; ?>">
				<br>
            </div>
            <div class="col-md-6">
				<label class="control-label">Landmark <span class="errormsg"  id="errlandmark"></span></label>
				<input type="text" name="landmark" id="landmark" class="form-control" placeholder="Landmark" value="<?php echo $rscustomer['landmark']; ?>">
				<br>
            </div>
            <div class="col-md-6">
				<label class="control-label">PIN code <span class="errormsg"  id="errpincode"></span></label>
				<input type="number" name="pincode" id="pincode" class="form-control" placeholder="Pincode" value="<?php echo $rscustomer['pincode']; ?>">
				<br>
            </div>
            <div class="col-md-6">
				<label class="control-label">Mobile number <span class="errormsg"  id="errmobile_no"></span></label>
				<input type="number" name="mobile_no" id="mobile_no" class="form-control" placeholder="Mobile number" value="<?php echo $rscustomer['mobile_no']; ?>">
				<br>
            </div>
        </div>
...
createOrder: function (data, actions) {
	...
	{
		document.getElementById("errfile").innerHTML ="Kindly Upload Profile Image....";	
		i=1;		
	}
	...
	{
		document.getElementById("errstate").innerHTML ="Kindly enter the state...";	
		i=1;		
	}
	...
	{
		document.getElementById("errpincode").innerHTML ="PIN code should contain number...";	
		i=1;		
	}
	...
	{
		document.getElementById("errmobile_no").innerHTML ="Mobile Number should contain number...";	
		i=1;		
	}
...
onApprove: function (data, actions) {
	return actions.order.capture().then(function (details) {
		...
		let myForm = document.getElementById('frmdata');
		let formData = new FormData(myForm);
		formData.append("payment_detail", details);
		jQuery.ajax({
			url: 'jswinningbid.php',
			data: formData,
			cache: false,
			contentType: false,
			processData: false,
			...
			success: function(paymentid){
				//alert("You have paid <?php echo $currencysymbol; ?> " + $('#paid_amount').val() + " successfully for winning bid...");
				window.location="paymentreceiptwinningbid.php?paymentid="+paymentid;
			}
		});
	})
},
onCancel: function (data) {
	window.location.replace("paywinningbid.php?status=failed" + "&winner_id=" + $('#winner_id').val())
}
