...
{   
	...
	// move_uploaded_file($_FILES["id_proof"]["tmp_name"],"img_id_proof/".$id_proof);
	$sql ="INSERT INTO customer(customer_fname,customer_lname,email_id,password,mobile_no,status,nin) values('$_POST[customer_fname]','$_POST[customer_lname]','$_POST[email_id]','$_POST[password]','$_POST[mobile_no]','Pending','$_POST[nin]')";
	$qsql = mysqli_query($con,$sql);
	if(mysqli_affected_rows($con) == 1)
	{
		echo "<script>alert('Registration Completed.. Administrator needs to verify your account. You will get notification after verification..');</script>";
		echo "<script>window.location='index.php';</script>";
	}
	else
	{
		echo "<script>alert('Failed to Register.. You have already registered your account in our system');</script>";
...
<form action="" method="post" enctype="multipart/form-data"> 
	...
	<p class="coupon-input form-row-last">
		<label>NIN <span class="required">*</span> <span class="errormsg"  id="errid_proof"></span></label>
		<input type="text"  name="nin" id="nin" pattern='[0-9]{11}'  />
	</p>
	<!-- <p class="coupon-input form-row-last">
		<label>ID Proof <span class="required">*</span> <span class="errormsg"  id="errid_proof"></span></label>
		<input type="file" name="id_proof" id="id_proof"  accept="image/*" >
	</p> -->
	<p> You're submitted i.d will be use for verfication purposes only </p>
   <div class="clear"></div>
	<p>
	<hr>
		<center><button value="Login" name="btnsubmit" id="btnsubmit" class="button-login" type="button" onclick="return validatecustomer()" >Click Here to Register</button></centeR>
...
function validatecustomer()
{
	...
	*/
	...
	{
		document.getElementById("errid_proof").innerHTML="Kindly enter your National Identification Number";
		errchk = "True";
	}	
	if(errchk == "True")
	{
	...
	else
	{
		...
		xmlhttp.onreadystatechange = function() {
			...
			{
				console.log('otp', this.responseText)
